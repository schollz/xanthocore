...
// static int InitDelayLine(dsy_Reverb2_dl *lp, int n);
...
void Reverb2::NextRandomLineseg(Reverb2Dl *lp, int n) {
  float prv_del, nxt_del, phs_inc_val;

  /* update random seed */
  if (lp->seed_val < 0) lp->seed_val += 0x10000;
...
int Reverb2::InitDelayLine(Reverb2Dl *lp, int n) {
  float read_pos;
  /* int     i; */
...
int Reverb2::Process(const float &in1, const float &in2, float *out1,
  ...
  if (lpfreq_ != prv_lpfreq_) {
    prv_lpfreq_ = lpfreq_;
    damp_fact = 2.0f - cosf(prv_lpfreq_ * (2.0f * (float)M_PI) / sample_rate_);
  ...
  for (n = 0; n < 8; n++) {
    ...
    if (lp->read_pos >= buffer_size) lp->read_pos -= buffer_size;
